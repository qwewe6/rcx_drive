...
|---|---|---|---|---|---|
| 1 | Backend platform: Supabase (hosted Postgres + PostGIS + Auth + Realtime) vs. a custom backend service (more control, more setup). See `docs/plans/initial-scaffolding.md` §6 for original framing; `docs/research/python-backend-react-native-stack.md` (2026-09-06) adds a Python/FastAPI custom-backend option to weigh, given both engineers' Python background and upcoming geospatial/ETL work. | Open | Max | — | — |
| 2 | Maps provider: Mapbox (better heatmap/style tooling, paid tiers) vs. MapLibre (open-source, no vendor lock-in, more setup work). | Open | Max | — | — |
